...
class MonitorStatus:
    ...
    def __next__(self):
        ...
        if self.status:  # monitor is on
            if on_condition:  # if monitor should on, sleep
                ...
                self.monitor_off_time = 0
                self.change_monitor_status(
                    status=0, mouse=self.debug, keyboard=self.debug)
            elif off_condition:
                self.change_monitor_status(
                    status=1, mouse=self.debug, keyboard=self.debug)
    ...
    def change_xinput_status(deviceId, status=1):
        ...
        """
        cmd = [
            'xinput', 'set-prop', str(deviceId), 'Device Enabled', str(status)
        ]
        subprocess.run(cmd, stdout=subprocess.PIPE)
